...
write_csv(dataWide, file = 'data/dataWideRaw.csv')
...
LongPlot <- dataLong |> 
  ...
  ggplot(aes(x = time, y = intensity, color = species))+
  geom_line(aes(group = sample_id))

ggsave('plots/basicLong.png', plot = LongPlot, dpi = 600, width = 10)
...
#Smoothing
library(prospectr)
metadata <- dfCorWide |> select(enzyme, instrument, species, time_point, sample)
spectra  <- dfCorWide |> select(-c(enzyme, instrument, species, time_point, sample)) |> as.matrix()


t_3 <- savitzkyGolay(spectra, p = 2, w = 151, m = 0)


smoothed_df <- bind_cols(metadata, as.data.frame(t_3))
...
write_csv(smoothed_df, file = 'data/dfCorSmoothWide.csv')
write_csv(smoothed_long, file = 'data/dfCorSmoothLong.csv')
...
t_4 <- savitzkyGolay(spectra, p = 2, w = 151, m = 0)


smoothed_df <- bind_cols(metadata, as.data.frame(t_4))
...
write_csv(smoothed_df, file = 'data/dfSmoothScaleWide.csv')
write_csv(smoothed_long, file = 'data/dfSmoothScaleLong.csv')
